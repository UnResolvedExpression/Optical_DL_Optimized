...
optimizableVariable('numHiddenUnits1',[60 300],'Transform','log','type','integer')
...
%tp=sum(class_test(:)== 2 == predicted_class(:))
%fp=sum(class_test(:)==1 && predicted_class(:)==2 && class_test(:)~=predicted_class(:))
tp=0;
fp=0;
fn=0;
for i = 1:length(class_test)
    if(class_test(i)==2 && predicted_class(i)==2)
        tp=tp+1;        
    end
    if(class_test(i)==1 && predicted_class(i)==2)
       fp=fp+1;        
    end
    if(class_test(i)==2 && predicted_class(i)==1)
        fn=fn+1;        
    end
...
fprintf('\nPrecision on test data is %5.2f%%',test_precision)
fprintf('\nRecall on test data is %5.2f%%',test_recall)
fprintf('\n')
